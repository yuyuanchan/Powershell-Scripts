...
function IsHoliday([DateTime]$InputDate) {
    ...
    $InputDateString = $InputDate.ToShortDateString()
...
#>
$InputDate = Get-Date # Get today's date
...
if ($ScheduledDate.ToShortDateString() -eq $InputDate.ToShortDateString()) { # If today is the scheduled day
    $Wshell = New-Object -ComObject Wscript.Shell # Creating a shell object
    $Subject = "Your task is due on $($ScheduledDate.ToShortDateString()) (today)" # Setting the message
